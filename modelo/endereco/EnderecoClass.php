...
class EnderecoClass {
    ...
    function DeletarRua($id) {     //função testada 100%
        $sql = "delete  from tb_rua where id_rua='$id'";
        $query = mysql_query($sql) or die("Impossivel deletar");
    }

    function DeletarUF($id) {    //função testada 100%
        $sql = "delete  from tb_uf where id_uf='$id'";
        $query = mysql_query($sql) or die("Impossivel deletar");
    }

    function DeletarBairro($id) {    //função testada 100%
    ...
    //################################funcoes das tabelas endereco e cidade ###########################################
    function CadastrarCidade() {
        $sql = "insert into tb_cidade (id_cidade,cid_nome,id_rua,id_bairro) values (null,'$this->cid_nome','$this->id_rua','$this->id_bairro') ";
        $query = mysql_query($sql) or die("erro ao gravar cidade");
    }

    function DeletarCidade($id) {
        $sql = "delete  from tb_cidade where id_cidade='$id'";
        $query = mysql_query($sql) or die("Impossivel deletar cidade!");
    }

    function UpdateCidade($id)
    {
        $sql = "update  tb_cidade set cid_nome='$this->cid_nome', id_rua='$this->id_rua', id_barro='$this->id_bairro'   where id_cidade='$id'";
        $query = mysql_query($sql) or die("Impossivel editar");    
    }
    ...
    //############################final funcoes tabela cidade #################################
    
    //############################funcoes tabela endereco #####################################
    function CadastrarEndereco()
    {
        $sql = "INSERT INTO `virtual-e`.`tb_endereco` (`id_endereco`, `end_cep`, `id_cidade`, `id_uf`) VALUES (NULL,'$this->end_cep','$this->id_cidade', '$this->id_uf')";
        $query = mysql_query($sql) or die("erro ao gravar endereco"); 
    }
    function DeletarEndereco($id) {
        $sql = "delete  from tb_endereco where id_endereco='$id'";
        $query = mysql_query($sql) or die("Impossivel deletar endereco!");
    }
      function ListarEndereco() 
    {
        ...
        {
            $sql = "update  tb_endereco set end_cep='$this->end_cep', id_cidade='$this->id_cidade', id_uf='$this->id_uf'   where id_endereco='$id'";
            $query = mysql_query($sql) or die("Impossivel editar");     
        }
